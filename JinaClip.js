javascript: (async () => {
  /* Jina Reader API Key (Replace with your actual API key) */
  const jinaApiKey = "Bearer XXX"; //  PUT YOUR API KEY HERE
  ...
  // Main content extraction using Jina Reader API
  async function getContentFromJina(url) {
    try {
      console.log("Fetching content from Jina API...");
      const response = await fetch(`https://r.jina.ai/${url}`, {
        method: "GET",
        headers: {
          Authorization: jinaApiKey,
      ...
      console.error("Error fetching content from Jina API:", error);
      return {
        title: "",
        content: "",
        chunk: [],
  ...
  // Get content using Jina API
  const jinaResponse = await getContentFromJina(document.URL);

  if (jinaResponse && jinaResponse.data) {
    const jinaData = jinaResponse.data; // Access the data object correctly
    console.log("jinaData", jinaData);

    // Extract title, content, and description from Jina API response
    let title = jinaData.title || getMetaContent("property", "og:title");
...
`;
    ...
    // Combine YAML and markdown content
    const fileContent =
      yamlFrontMatter + "# " + sanitizedTitle + "\n" + markdownBody;
    ...
    console.error("Error: Jina API response is invalid or missing data.");
  }
})();
