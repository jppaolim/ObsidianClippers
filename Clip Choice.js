...
const {
 ...
 function fixMarkdownLinks(text) {
  ...
  const fileName = `${today}-${sanitizedTitleForFile}`;
    ...
    // Determine if we're in metadata-only mode (you can modify this based on your needs)
    // For example, you could add a prompt or keyboard shortcut to toggle this
    const isMetadataOnly = confirm("Save as metadata-only (link)?");
  ...
  /* YAML front matter as tags render cleaner with special chars */
   ...
   '---\n\n';
    ...
    if (isMetadataOnly) {
        ...
        const selection = getSelectionHtml();
        const markdownify = selection || cleanupHTML(content);
        const turndownService = createTurndownService();
        const markdownBody = turndownService.turndown(markdownify);
        fileContent = baseYAML + "# " + sanitizedTitle + '\n' + markdownBody;
    }
